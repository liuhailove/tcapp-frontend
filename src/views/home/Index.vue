    ...
    <div id="top-bar">
      <van-sticky>
          <van-tabs v-model:active="active" lazy-render @change="handleTabClick" sticky>
            <van-tab v-for="(item,idx) in topBar" :name="item.id">
              <template #title>
...
// 默认活跃的tab
const active = markRaw(5);
// 常用功能
const showCommonFeatures = markRaw(false);
// 最后活跃的tab
let lastActive = active.value;
// 活跃组件
const activeTabComponent = markRaw(LiveContent);
/**
 * 处理点击事件
 * @param id 点击的id
 */
